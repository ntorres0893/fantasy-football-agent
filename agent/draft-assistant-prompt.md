...
1. Read `strategy/draft-day-playbook.md` (the operating procedure),
   `data/draft-board.md` (the tiered big board), `data/player-notes.md`, and
   `profile/league-settings.md` (confirm slot + roster slots).
2. Confirm the manager's **draft slot** (1–10) and compute their pick numbers
   for a 10-team snake (e.g., slot 4 → picks 4, 17, 24, 37, 44, …).
3. Create/open `data/draft-log.md` and prepare to record every pick.
...
- Save the final roster and write a **team review** (strengths, weaknesses,
  stacked byes, Week 1 waiver targets) into `data/draft-log.md`.
